...
docker run -d --name blog --restart always -p 122:22 -p 443:443 -e "DOMAIN=$DOMAIN" -v `pwd`/html:/nginx/html -v `pwd`/acme_ssl:/root/.acme.sh/$DOMAIN -e "AUTHORIZED_KEYS=$AUTHORIZED_KEYS" -e DNS_TYPE=$DNS_TYPE -e DP_Id=$DP_Id -e DP_Key=$DP_Key blademainer/hexo-nginx-https:v0.0.3
